...
class StatisticsCalculatorBackendApplicationTests {
	...
	public void testCalcQuantile(){
		...
		double[] result_negativ = {-4,7, -4,4, -3,5, -1, -1, -1};
		...
		for (int i=0; i<quantilen_optimal.length; i++){
			quantilen_optimal[i] = calculations.calcQuantile(percentages[i], W_Optimal);
		}

		double[] quantilen_negativ = new double[6];
		for (int i=0; i<quantilen_negativ.length; i++){
			quantilen_negativ[i] = calculations.calcQuantile(percentages[i], W_Negativ);
		}

		double[] quantilen_null = new double[6];
		for (int i=0; i<quantilen_null.length; i++){
			quantilen_null[i] = calculations.calcQuantile(percentages[i], W_Null);
		}

		double[] quantilen_einzelwert = new double[6];
		for (int i=0; i<quantilen_einzelwert.length; i++){
			quantilen_einzelwert[i] = calculations.calcQuantile(percentages[i], W_Einzelwert);
		}

		double[] quantilen_gleich = new double[6];
		for (int i=0; i<quantilen_gleich.length; i++){
			quantilen_gleich[i] = calculations.calcQuantile(percentages[i], W_Gleich);
		}

		Assertions.assertEquals(result_optimal, quantilen_optimal);
		Assertions.assertEquals(result_negativ, quantilen_negativ);
		Assertions.assertEquals(result_null, quantilen_null);
		Assertions.assertEquals(result_einzelwert, quantilen_einzelwert);
		Assertions.assertEquals(result_gleich, quantilen_gleich);
	}
